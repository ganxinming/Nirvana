...
对象分配空间(新生代[eden,from,to]，老年代)，所有的对象实例以及数组，字符串常量池都要在堆上分配，但是随着 JIT 编译器的发展与逃逸分析技术逐渐成熟，栈上分配、标量替换优化技术将会导致一些微妙的变化发生，所有的对象都分配在堆上也渐渐变得不是那么“绝对”了。
...
1.当前运行活跃线程中的虚拟机栈变量表引用的对象

2.方法区常量引用的对象

3.方法区中的类静态变量(static)引用的对象

4.本地方法栈中的JNI(Native方法)的引用对象

5.被启动类（bootstrap加载器）加载的类和创建的对象
